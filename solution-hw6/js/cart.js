...
function remove() {
    if (cart.length != 0) {
        ...
        // Gets where the item is in the cart
        let index = getItemIndex(selected);
        // Removes the item at the specific index from the cart
        removeFromCart(index);
        // Saves the changes to the storage
        saveToLocalStorage();
        // Removes the item from the display
        selected.remove();
